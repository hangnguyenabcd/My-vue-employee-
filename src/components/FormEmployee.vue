<template>
  <div>
    <v-card class="wrapper margintop" >
        <v-card-title>
        <span class="headline" v-if="isCreate">New Employee</span>
        <span class="headline"  v-else>Update Employee</span>
        </v-card-title>
      <v-container grid-list-md >
        <v-form>
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="blue darken-1" flat @click="cancelclicked">Cancel</v-btn>
...
export default {
  created(){
        ...
        EmployeeRepository.getUserById(this.employee.id).then(res=>{
            this.employee=res
            var monthName = ['January', 'February','March','April','May','June','July','August', 'September', 'October', 'November', 'December']
            var date = new Date(res.birthday)
            this.birthday.day= date.getDate().toString()
            this.birthday.month = monthName[date.getMonth()].toString()
            this.birthday.year = date.getFullYear().toString()
      })
        }
  ...
  methods: {
    ...
    saveClicked: function() {
      ...
      if(!this.isCreate){
        EmployeeRepository.update(this.employee.id,this.employee).then(res =>{
        console.log(res);
        this.$router.push({name: 'Employee'})
      })
      }
